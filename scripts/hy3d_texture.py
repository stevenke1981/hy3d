...
def main() -> int:
    ...
    try:
        if args.max_views < 6 or args.max_views > 12:
            message = "--max-views must be between 6 and 12"
            print(f"error: {message}", file=sys.stderr)
            return finish("error", 2, message)
        if not mesh_path.exists():
            message = f"mesh not found: {mesh_path}"
            print(f"error: {message}", file=sys.stderr)
        ...
        if not paint_model.exists():
            message = f"paint model not found: {paint_model}"
            print(f"error: {message}", file=sys.stderr)
        ...
        if not realesrgan_ckpt.exists():
            message = f"RealESRGAN checkpoint not found: {realesrgan_ckpt}; download from {REALESRGAN_URL}"
            print(f"error: {message}", file=sys.stderr)
            return finish("error", 7, message)
        ...
        config = Hunyuan3DPaintConfig(args.max_views, args.resolution)
        ...
        if not produced_obj.exists():
            message = f"texture pipeline did not produce expected OBJ: {produced_obj}"
            print(f"error: {message}", file=sys.stderr)
